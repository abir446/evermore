...
export const NavBody = ({ children, className, visible }) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-pink-200 px-4 py-2 lg:flex",
        className
      )}
    >
      {children}
      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-pink-300"
            />
          )}
...
export const MobileNav = ({ children, className, visible }) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-50 mx-auto flex w-full max-w-[calc(100vw-2rem)] flex-col items-center justify-between bg-pink-200 px-0 py-2 lg:hidden",
        className
      )}
    >
      {children}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-pink-100 px-4 py-8 shadow-[0_0_24px_rgba(34,_42,_53,_0.06),_0_1px_1px_rgba(0,_0,_0,_0.05),_0_0_0_1px_rgba(34,_42,_53,_0.04),_0_0_4px_rgba(34,_42,_53,_0.08),_0_16px_68px_rgba(47,_48,_55,_0.05),_0_1px_0_rgba(255,_255,_255,_0.1)_inset]",
            className
          )}
        >
          {children}
...
export const NavbarButton = ({
  ...
  const variantStyles = {
    ...
    gradient:
      "bg-gradient-to-b from-pink-400 to-pink-600 text-white shadow-[0px_2px_0px_0px_rgba(255,255,255,0.3)_inset]",
  };
